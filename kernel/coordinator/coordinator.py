...
class KernelCoordinator:
    ...
    Отвечает за:
    - приём входящих запросов
    - создание задачи
    - управление жизненным циклом (через TaskManager)
    - формирование execution context
    - вызов execution слоя (Executor)
    - сбор timing и tracing информации

    НЕ отвечает за:
    - выполнение задач (это делает Executor)
    - хранение данных (это делает TaskManager / БД)
    """
    ...
    def __init__(self):
        ...
        # Менеджер задач (FSM + хранение)
        self.task_manager = TaskManager()

        # Executor (стратегия выполнения)
        self.executor = LLMExecutor()

        # Построитель execution context
    ...
    async def handle_request(self, input_data: str):
        ...
        6. Выполнение executor
        7. Сохранение результата
        8. Логирование trace

        :param input_data: вход пользователя
        :return: Task (с результатом и trace)
        """
        ...
        try:
            ...
            logger.info(
                f"[CONTEXT BUILT] task_id={task.id} model={context.get('model')}"
            )
        ...
        # сохраняем trace в контекст задачи (очень важно для дебага)
